...
Adafruit_BMP280 bme;
#define ALTITUDE 79 // Altitude of my home
...
void loop()
{
    //pressure1 = random(1011, 1022);
  pression_atm = bme.readPressure() / 100.0;
  //pression_atm = pressure1;
...
void InitLcd()
{
    ...
    myGLCD.print("Weather Station Ver. 1.0a", CENTER, 10);
    ...
    myGLCD.drawLine(570, 330, 570, 475);
    ...
    myGLCD.setFont(BigFont);
    myGLCD.print("In Max:", 10, 414);
    myGLCD.print("In Min:", 10, 448);
    myGLCD.print("Out Max:", 10, 340);
    myGLCD.print("Out Min:", 10, 379);
    myGLCD.print("DewPoint:", 300, 340);
    myGLCD.print("HeatIndex:", 300, 379);
    myGLCD.print("Ins Humid:", 300, 414);
    //myGLCD.print("Min In:", 300, 448);
...
void Time(){
    ...
    //myFiles.load(0, 0, 150, 110, "w21.RAW", 1, 0);
    
    char bufferLP[16];
    LP.toCharArray(bufferLP, 16);
    myGLCD.setFont(SmallFont);
    myGLCD.setColor(255, 255, 255);
    myGLCD.print(bufferLP, 670, 340);
    Serial.print("Moon Phase");
    Serial.println(bufferLP);
    Serial.print("Lunar Cycle");
  ...
  myGLCD.print(weather_text, 300, 225);
...
void Temp_Int()
{
  ...
  //myGLCD.setFont(BigFont);
  myGLCD.setFont(Grotesk16x32);
  myGLCD.printNumF(hum_Int, 1, 460, 405);
  myGLCD.print("%",530,405);
  myGLCD.printNumF(Tmax_Int, 1, 170, 405);
  myGLCD.printNumF(Tmin_Int, 1, 170, 440);
...
void Temp_Ext ()
{
    ...
    myGLCD.setFont(Grotesk16x32);
    myGLCD.printNumF(dPF, 1, 460, 335);
    myGLCD.printNumF(heatIndex(tF,h), 1, 460, 370);
    myGLCD.setFont(SmallFont);
  ...
  myGLCD.setFont(Grotesk16x32);
  myGLCD.printNumF(Tmax_Ext, 1, 170, 335);
  myGLCD.printNumF(Tmin_Ext, 1, 170, 370);
  myGLCD.printNumF(pression_atm, 1, 10, 260);
}
...
void hist_pressure(int x, int position_h )
{
  int larg = 15;
  int hauteur ;
  hauteur = 311 - (history[position_h] - 980);
  myGLCD.setColor(VGA_BLUE);
  myGLCD.fillRect(x, 311, x + larg, hauteur);
  //Serial.println(hauteur);
  myGLCD.setColor(VGA_BLACK);
  myGLCD.fillRect(x, (hauteur - 1), x + larg, 222);
}
...
float read_p(){
  int reading = (bme.readPressure()/100.0+pressure_offset)*10; // Rounded result to 1-decimal place
...
String get_trend_text(float trend){
  ...
  else if (trend >= -1.5  && trend < -0.25) { trend_str = "Fall slow  "; }
  else if (trend >= -3.5  && trend < -1.5)  { trend_str = "Falling    ";      }
  else if (trend <= -3.5)                   { trend_str = "Fall fast  "; }
  return trend_str;
}
